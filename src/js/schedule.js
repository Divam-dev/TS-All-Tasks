...
function validateLesson(lesson) {
    const conflictingLesson = schedule.find(existingLesson => existingLesson.dayOfWeek === lesson.dayOfWeek &&
...
export function reassignClassroom(dayOfWeek, timeSlot, oldClassroomNumber, newClassroomNumber) {
    const lessonIndex = schedule.findIndex(lesson => lesson.dayOfWeek === dayOfWeek &&
        lesson.timeSlot === timeSlot &&
        lesson.classroomNumber === oldClassroomNumber);
    if (lessonIndex === -1) {
        return false;
    }
    const isNewClassroomFree = !schedule.some(lesson => lesson.dayOfWeek === dayOfWeek &&
        lesson.timeSlot === timeSlot &&
